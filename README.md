...
| **auditd + audisp plugin (신규)** | `auditd/` | auditd 3.0+, Go (빌드 시) |
...
| `/etc/audit/rules.d/audit-cmd-logging.rules` | auditd 감사 규칙 |
| `/etc/audit/plugins.d/audit-cmd-logger.conf` | audisp 플러그인 설정 |
...
# audisp 플러그인 설정 확인
cat /etc/audit/plugins.d/audit-cmd-logger.conf
